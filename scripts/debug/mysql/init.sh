...
mysql -u ${MYSQL_ROOT_USER} --password="${MYSQL_ROOT_PASSWORD}" ${MYSQL_DATABASE} -e "CREATE TABLE IF NOT EXISTS clients_schema.plans (id VARCHAR(36), duration INT NOT NULL, tier ENUM('FREE') UNIQUE NOT NULL);"
...
mysql -u ${MYSQL_ROOT_USER} --password="${MYSQL_ROOT_PASSWORD}" ${MYSQL_DATABASE} -e "INSERT INTO clients_schema.plans (id, duration, tier) VALUES ('5532b773-be2f-42e5-a4da-a0856bf0d62c', 3600000, 'FREE');"
...
mysql -u ${MYSQL_ROOT_USER} --password="${MYSQL_ROOT_PASSWORD}" ${MYSQL_DATABASE} -e "CREATE TABLE IF NOT EXISTS clients_schema.quota (client_id VARCHAR(36), plan_id VARCHAR(36) NOT NULL, quota INT NOT NULL, refresh_in BIGINT NOT NULL);"
...
mysql -u ${MYSQL_ROOT_USER} --password="${MYSQL_ROOT_PASSWORD}" ${MYSQL_DATABASE} -e "CREATE TABLE IF NOT EXISTS short_urls_schema.short_urls (id VARCHAR(10), client_id VARCHAR(36) NOT NULL, long_url TEXT NOT NULL, created_at BIGINT NOT NULL);"
...
mysql -u ${MYSQL_ROOT_USER} --password="${MYSQL_ROOT_PASSWORD}" ${MYSQL_DATABASE} -e "GRANT SELECT, INSERT ON short_urls_schema.short_urls TO '${MYSQL_USER}'@'%';"
